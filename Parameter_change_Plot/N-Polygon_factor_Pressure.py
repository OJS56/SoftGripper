...
filename = 'var_n_data_h18mm.xlsx'
...
for i in range(5):
    ...
    for pressure in pressures:
        pressure_data = df.loc[pressure, [f"Weight_{exp_case}_n{4+(i*2)}.1", f"Weight_{exp_case}_n{4+(i*2)}.2", f"Weight_{exp_case}_n{4+(i*2)}.3"]]
    ...
    # errorbar 그리기
    plt.errorbar(p_num, mean, yerr=std, fmt=f'-{markers[i]}', capsize=5, label=f'N = {4+2*i}', linewidth = 0.5, markersize = 4, ecolor=colors[i], color=colors[i])

plt.xlabel('Pressure (kPa)', fontsize = 16)
plt.ylabel('Force (N)', fontsize = 16)
plt.title(f'N-Polygon UP(UpPositive)', fontsize = 18)
plt.legend(title = "N-Polygon", title_fontsize = 12)
plt.grid(True)
# plt.show()
name_to_save = input('press file name to be saved: ')
fig.savefig(name_to_save, dpi = 600)