...
for i in range(7):
    ...
    # errorbar 그리기
    plt.errorbar(p_num, mean, yerr=std, fmt=f'-{markers[i]}', capsize=5, label=f'H = {3*(i+1)+6}', linewidth = 0.5, markersize = 4, ecolor=colors[i], color=colors[i])

plt.xlabel('Pressure (kPa)', fontsize = 16)
plt.ylabel('Force (N)', fontsize = 16)
plt.title(f'Height UP(UpPositive)', fontsize = 18)
plt.legend(title = "Height (mm)", title_fontsize = 12)
plt.grid(True)
# plt.show()
