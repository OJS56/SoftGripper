...
filename = 'var_diam_data_h18mm.xlsx'
...
for i in range(6):
    ...
    # errorbar 그리기
    plt.errorbar(p_num, mean, yerr=std, fmt=f'-{markers[i]}', capsize=5, label=f'D = {18+3*(i)}', linewidth = 0.5, markersize = 4, ecolor=colors[i], color=colors[i])


plt.xlabel('Pressure (kPa)', fontsize = 16)
plt.ylabel('Force (N)', fontsize = 16)
plt.title(f' Diameter DN(DownNegative)', fontsize = 18)
plt.legend(title = "Diameter (mm)", title_fontsize = 12)
plt.grid(True)
# plt.show()
